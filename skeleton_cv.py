...
def calculate_angle(line):
    ...
    angle = np.clip(angle, -np.pi/2, np.pi/2) # TODO

    return angle

def line_point_distance(a, b, x, y):
        
    distance = float(np.sqrt((y-a*x-b)**2)/np.sqrt(a**2+b**2)) # TODO

    return distance

def fit_line(points_list):
    if len(points_list) < 2: return 0, 0
    ...
    if den == 0: # 수직선인 경우 (기울기 무한대 처리 대안)
        return 9999, -9999 * x_bar 
